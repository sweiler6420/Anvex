...
| A command in the `web` container cannot write into `/app`, but only on Linux | `web` runs as uid 1000 and compose bind-mounts the host's `frontend/` over `/app`. Windows mounts ignore ownership; Linux does not, and a CI checkout belongs to uid 1001 | write to `/tmp`, never back through the bind mount — `cacheDir`, `beat --schedule` and the smoke's bundle all already do (ANV-41) |

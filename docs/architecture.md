...
| `POST` | `/v1/auth/refresh` | JSON body. Mints a **new** pair. The presented token is **not** revoked — §6. |
...
| **A refresh token carries no `jti`, and two things follow.** A spent one is never revoked — `POST /v1/auth/refresh` mints a new pair and the token presented keeps working for its whole `JWT_REFRESH_TOKEN_EXPIRE_MINUTES` lifetime, because there is nowhere to record that it has been used. And the "new" token is not necessarily a different string: the payload is `{sub, type, iat, exp}` at one-second resolution, so a rotation in the same second as the mint before it is byte-identical. Rotation therefore bounds a *stolen* token only loosely. **Both found by ANV-41's smoke run** — the first asserted the contract this document used to state and got a 200; the second failed intermittently until the cause was understood. | `backend/app/services/auth.py` | `TODO(ANV-refresh-revocation)` |
